...
def call_tool(file_path, tool_index):
    if tool_index == "1a":
        ...
        result = subprocess.run(['Rscript', '/analyzer/r-files/t-test.R', file_path, "0.09"], capture_output=True, text=True)
        return result.returncode
    return None
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        ...
        print("  <tool_index>     : Index of the tool to use.")
        print("                     1a : rtlf-alpha<<0.9%, 1b : rtlf-alpha<0.9%, 1c : rtlf-alpha<9%, 1d : rtlf-alpha<18%, 2: dudect, 3: mona, 4: t-test")
